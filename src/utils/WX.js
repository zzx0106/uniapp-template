...
export default {
  ...
  async getOpenId() {
    try {
      ...
      const openId = await api_getOpenId({ code });
      return openId;
    } catch (err) {
      this.Toast('登录失败');
   ...
   * @returns {Promise}
   */
  navigateTo: throttle(function(url, data) {
   ...
   * @returns {Promise}
   */
  redirectTo: throttle(function(url, data) {
    if (!url || typeof url !== 'string') {
   ...
   * 如果回退到栈首，那么就跳转home页
   */
  navigateBack: throttle(function(delta = 1) {
    uni.navigateBack({
      delta,
      fail: (err) => {
   ...
   * @returns {Promise}
   */
  reLaunch: throttle(function(url, data) {
    if (!url || typeof url !== 'string') {
   ...
   * @returns {Promise}
   */
  switchTab: throttle(function(url) {
    return new Promise((resolve, reject) => {
      uni.switchTab({
        url,
        success(res) {
